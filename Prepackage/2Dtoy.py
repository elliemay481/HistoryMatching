...
def history_match(parameter_bounds, sigma_cov, var_exp, beta, ndim, N_training_pts, N_test_pts, waves=1):
    ...
    input_train_all, input_test_all = data.prepare_data(ndim, Nsamples, Ntraining, parameter_bounds)
    ...
    for k in range(waves):
        ...
        for n in range(N_regions):
            ...
            for i in range(N_regions):
                ...
                output_train_i = np.zeros(N_training_pts)
                for i in range(N_training_pts):
                    output_train_i[i] = true_model(input_train_i[i,0], input_train_i[i,1])
